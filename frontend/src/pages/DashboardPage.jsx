...
import { Activity, Heart, Droplets, Zap, TrendingUp, ArrowRight, Award } from 'lucide-react'
import { modelsAPI } from '../api/client'
import { useAuthStore } from '../store/authStore'

const DiseaseCard = ({ disease, emoji, color, label, description, bestModel, bestAuc, onClick }) => {
  const colorMap = {
...
export default function DashboardPage() {
  ...
  const getBest = (disease) => {
    if (!perfData?.[disease]) return {}
    const entries = Object.entries(perfData[disease])
  ...
  const totalPredictions = 0 // Would come from history API in a full impl
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={TrendingUp} label="Best AUC"          value={perfData ? Math.max(
          parseFloat(heartBest.auc || 0),
          parseFloat(diabetesBest.auc || 0),
          parseFloat(kidneyBest.auc || 0)
        ).toFixed(3) : '—'} sub="Across all models" color="bg-emerald-50 text-emerald-600" />
        <StatCard icon={Heart}      label="Total Predictions" value={totalPredictions || '—'} sub="All time" color="bg-rose-50 text-rose-600" />
      </div>
      ...
      <div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          <DiseaseCard
            ...
            bestModel={heartBest.model}
            bestAuc={heartBest.auc}
          ...
          <DiseaseCard
            ...
            bestModel={diabetesBest.model}
            bestAuc={diabetesBest.auc}
          ...
          <DiseaseCard
            ...
            bestModel={kidneyBest.model}
            bestAuc={kidneyBest.auc}
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-brand-200 text-sm max-w-lg">
              Enter patient clinical data → choose a disease → select a model (or let the system auto-select the best one).
              Predictions include probability scores and SHAP-based explanations showing which features drove the result.
            </p>
          </div>
          <Activity className="w-10 h-10 text-brand-300 opacity-60 hidden md:block" />
        </div>
        <div className="flex flex-wrap gap-2 mt-4">
          {['SMOTE Balancing', 'SHAP Explainability', 'Cross-Validation', '6 ML Models', 'PDF Reports'].map((tag) => (
            <span key={tag} className="px-3 py-1 bg-white/10 rounded-full text-xs font-medium text-brand-100">{tag}</span>
          ))}
        </div>
      </div>
    </div>
